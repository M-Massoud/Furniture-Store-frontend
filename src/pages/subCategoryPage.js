...
export default function SubCategoryPage() {
  ...
  const [subCategoryProductsPageData, setSubCategoryProductsPageData] = useState([]);
  ...
  useEffect(() => {
    ...
    // .get(`/${keyword}`)
      .then(res => {
        ...
        setMaxPagesNumber(Math.ceil(numberOfProductsInSubCategories / maxItemsNumberInPage));
        const startFromSelectedItemId = currentPage * maxItemsNumberInPage - maxItemsNumberInPage;
        const endToSelectedItemId = currentPage * maxItemsNumberInPage;
        let newArr = [...res.data.products];
        setSubCategoryProductsPageData(newArr.slice(startFromSelectedItemId, endToSelectedItemId));
      })
