...
export default function LoginForm() {
    ...
    return (
        <div className="row flex-space-around my-5">
            <div className="bg-main m-5 p-5 col-10 col-xl-4 border-custom shadow-lg">
                <h3 className="text-main mb-2">Registered Customers</h3>
                <h6 className="mb-4">If you have an account, sign in with your email address.</h6>
                <form onSubmit={(e) => handleSubmit(e)}>
                    <div className="mb-3">
                        <label htmlFor="userEmail" className="form-label font-bold">Email Address</label>
                        <input type="email" className={`form-control ${loginDataErrors.userEmailError ? "border-danger" : ""}`} name="userEmail" aria-describedby="userEmailHelp" value={loginData.email} onChange={(event) => { changeLoginData(event) }} placeholder="example@email.com" />
                        <div id="userEmailHelp" className="form-text text-danger">{loginDataErrors.userEmailError}</div>
                    </div>
                    <div className="form-group mb-3">
                        <label htmlFor="userPassword" className="form-label font-bold">Password</label>
                        <div className="input-group relativePosition">
                            <input type={isShownPassword ? "text" : "password"} className={`form-control ${loginDataErrors.userPasswordError ? "border-danger" : ""}`} name="userPassword" aria-describedby="userPasswordHelp" value={loginData.password} onChange={(event) => { changeLoginData(event) }} />
                        ...
                        <div id="userPasswordHelp" className="form-text text-danger">{loginDataErrors.userPasswordError}</div>
                    </div>
                    <Link to={'/forgetPassword'} className="btn mb-2 text-hover-red">Forgot Your Password?</Link>
                    <button type="submit" className="btn bg-secondary-1 white col-12">Login</button>
                </form>
            </div>

            <div className="bg-main m-5 p-5 col-10 col-xl-4 border-custom shadow-lg">
                <h3>Create Your HUB Furniture Account</h3>
                <Link to={'/register'} className="btn bg-secondary-1 white mt-3 col-12">Sign up</Link>
