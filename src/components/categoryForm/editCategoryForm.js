...
export default function EditCategoryForm(props) {
    ...
    const [formDetails, setFormDetails] = useState({
        categoryTitle: props.location.state.title,
        categorySubCategory: props.location.state.subCategory,
    });
        ...
        if (event.target.name === "categorySubCategory") {
            ...
            if (!dublicateIndicator) {
                setFormDetails({
                    ...formDetails,
                    [event.target.name]: [
                        ...formDetails.categorySubCategory,
                        { _id: subCategoriesData[event.target.value]._id, title: subCategoriesData[event.target.value].title }
                    ],
                });
            }
        }
    ...
    return (
        <>
            <div className='col-10 offset-1 col-md-8 offset-md-2 border p-3 rounded shadow my-5'>
                ...
                <h6 className='text-center mt-2'>Edit Category</h6>
                ...
                <form className={`co-12  row m-auto `} onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <select className={`form-control form-select ${formError.categorySubCategoryError && "border-danger"} `} id={'categorySubCategory'} name={'categorySubCategory'} onChange={(event) => handelFormchange(event)} required>
                    ...
                    <button type="submit" className="btn bg-secondary-1 white my-4 w-50 m-auto"> Edit Category </button>
                </form>
            </div>
        </>
    );
